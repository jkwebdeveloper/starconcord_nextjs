...
const AboutUs = () => {
  ...
  const firstTwoUsps = useMemo(() => aboutUs?.uspsList?.slice(0, 2), [aboutUs]);
  const remainingUsps = useMemo(() => aboutUs?.uspsList?.slice(2), [aboutUs]);
  return (
            ...
            <div className="w-full text-[#1B1B1B] space-y-20">
              <div className="space-y-10">
                ...
                <p className="text-4xl font-bold">
                  {aboutUs?.aboutContentTitle}
                </p>
                <div
                  className="tag"
                  dangerouslySetInnerHTML={{ __html: aboutUs?.aboutContent }}
                ></div>
                {aboutUs.ceoName && (
              ...
              <div className="space-y-8">
                ...
                <div className="space-y-6">
                  {selectedTab === "mission" && (
                    <div className="space-y-8">
                      <div
                        className="space-y-3 hcontent tag"
                        dangerouslySetInnerHTML={{
                          __html: aboutUs?.ourMission,
                        }}
                      ></div>
                    </div>
                  )}
                  {selectedTab === "vision" && (
                    <div className="space-y-8">
                      <div
                        className="space-y-3 hcontent tag"
                        dangerouslySetInnerHTML={{ __html: aboutUs?.ourVision }}
                      ></div>
          ...
          <div className="grid items-start justify-center gap-5 px-3 lg:px-10 lg:grid-cols-2 xl:gap-40 md:gap-20">
            ...
            <div className="space-y-8">
              ...
              <div
                ...
                dangerouslySetInnerHTML={{ __html: aboutUs.whyusContent }}
              ></div>
              <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
                {aboutUs?.whyusPoints?.map((item1) => (
                  <div className="flex items-center gap-3" key={item1._id}>
                    ...
                    <p className="text-xl font-semibold">{item1?.name}</p>
                  </div>
                ))}
              </div>
            </div>
                  ...
                  <div className="w-full">
                    {aboutUs.certificateList?.map((item) =>
                      membershipTab === item.title ? (
                        <div key={item._id}>
                          <div
                            className="text-lg hcontent tag"
            ...
            <div className="grid items-start justify-center gap-5 px-3 lg:px-10 lg:grid-cols-2">
              <div className="space-y-3">
                ...
                <div
                  ...
                  dangerouslySetInnerHTML={{ __html: aboutUs?.uspsDescription }}
                ></div>
              </div>
