...
const GetInTouchSection = () => {
  ...
  const handleGetContact = () => {
    setLoading(true);
    axios("https://starconcord.com.in/scbk/api/contactUs", {
      headers: {
  ...
  const handlePost = (values, actions) => {
    setLoading(true);
    axios("https://starconcord.com.in/scbk/api/send-meesage", {
      method: "post",
      data: {
        email: values.email,
        name: values.name,
      ...
      <div className="grid items-center gap-5 lg:grid-cols-2 xl:gap-40 md:gap-20">
        <div className="space-y-5 lg:space-y-8">
          ...
          <p className="text-[#6C6C6C] text-sm">
            Our team of experts is ever ready to assist you by offering the best
            freight forwarding solutions for your needs. Get in touch with us
            today.
          </p>
          <div className="space-y-2">
            ...
            <p className="text-lg font-bold w-[70%]">{contact?.address}</p>
          </div>
        </div>
